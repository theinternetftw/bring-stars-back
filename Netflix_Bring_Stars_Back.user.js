...
// OPTIONS: right now just edit these by hand
var bsbOptions = {
...
function loadRatingsCache() {
  ...
  var cache = cacheJSON ? JSON.parse(cacheJSON) : {};
  var now = Date.now();
  for (var c in cache) {
...
function saveRatingsCache() {
  GM_setValue('bsbRatingsCache', JSON.stringify(ratingsCache));
}

var ratingsCache = loadRatingsCache();
